...
pub async fn handle_messages(
    ...
    // Log incoming request path for debugging/visibility (consumed by other apps via cc-switch.log tailing).
    if state.config.read().await.enable_logging {
        let path = uri
            .path_and_query()
            .map(|pq| pq.as_str())
            .unwrap_or(uri.path());
        log::info!("[REQ] app=claude path={path}");
    }
...
pub async fn handle_chat_completions(
    ...
    if state.config.read().await.enable_logging {
        let path = uri
            ...
            .map(|pq| pq.as_str())
            .unwrap_or(uri.path());
        log::info!("[REQ] app=codex path={path}");
    }
...
pub async fn handle_gemini(
    ...
    if state.config.read().await.enable_logging {
        let path = uri
            ...
            .map(|pq| pq.as_str())
            .unwrap_or(uri.path());
        log::info!("[REQ] app=gemini path={path}");
    }
